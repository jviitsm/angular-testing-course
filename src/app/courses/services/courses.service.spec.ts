...
describe("CoursesService", () => {
  ...
  it("should modify partially a course", () => {
    const changes: Partial<Course> = {
      titles: { description: "changed title" },
    };

    coursesService.saveCourse(12, changes).subscribe((changedCourse) => {
      expect(changedCourse.id).toBe(12);
      expect(changedCourse.titles.description).toEqual(
  ...
  it("should give an error if save course fails", () => {
    ...
    coursesService.saveCourse(12, changes).subscribe(
      () => fail("the save course operation shoud have failed"),
